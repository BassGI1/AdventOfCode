puzz = """|-L.FL-FJFL.|7FJ7F|F7-7F77.FL7---.7FF77--.|-F|FF--7FF7F-7F--77FFFJFJF7.F-LF7LFJJ.F|.|77F7F---7FLL.|FFJ.FF7-77..F77.-F-7FL-FF|77F-FFLL-FJ-|.J
...
7JFF.FF-L-LJJJ-7JL7LLL|7JJ.LJJLLLFJLFJ7JJL.LL--JJLJ..JLL-FF---J.J.F.L.J.|-|LFF--7-L|-LF.J-JJJLLF--L-|J.L|L|.J-J.LLJJ-L|.|-L-F---J.LJJJ.FJL|7""".split("\n")

# map works like this:
# for each allowed character, the first set is the top allowed, next is right, then bottom, then left
# kinda like css
...
allowedConnections = {
	"|": [top, set(), bottom, set()],
	"F": [set(), right, bottom, set()],
	"-": [set(), right, set(), left],
	"L": [top, right, set(), set()],
	"J": [top, set(), set(), left],
	"7": [set(), set(), bottom, left],
	"F": [set(), right, bottom, set()],
	".": [set(), set(), set(), set()],
...
currentNode = startCoords
iterator = 0

while 1:
	mainLoop.add(currentNode)
	if currentNode[1] > 0 and grid[currentNode[1] - 1][currentNode[0]] in allowedConnections[grid[currentNode[1]][currentNode[0]]][0] and (currentNode[0], currentNode[1] - 1) not in mainLoop:
		iterator += 1
		currentNode = (currentNode[0], currentNode[1] - 1)
		continue
	if currentNode[1] < len(grid) - 1 and grid[currentNode[1] + 1][currentNode[0]] in allowedConnections[grid[currentNode[1]][currentNode[0]]][2] and (currentNode[0], currentNode[1] + 1) not in mainLoop:
		iterator += 1
		currentNode = (currentNode[0], currentNode[1] + 1)
		continue
	if currentNode[0] > 0 and grid[currentNode[1]][currentNode[0] - 1] in allowedConnections[grid[currentNode[1]][currentNode[0]]][3] and (currentNode[0] - 1, currentNode[1]) not in mainLoop:
		iterator += 1
		currentNode = (currentNode[0] - 1, currentNode[1])
		continue
	if currentNode[0] < len(grid[0]) - 1 and grid[currentNode[1]][currentNode[0] + 1] in allowedConnections[grid[currentNode[1]][currentNode[0]]][1] and (currentNode[0] + 1, currentNode[1]) not in mainLoop:
		iterator += 1
		currentNode = (currentNode[0] + 1, currentNode[1])
		continue
	break
# my code only does lines 2 to n - 1, lines 1 and n were done raw dogged for p1
# format of string is messed up in vscode for some reason

puzz = """......124..................418.......587......770...........672.................564............................438..........512......653....
...
.......877......256.714...................825.........458....................869..............................54............28.823..110.....""".split("\n")
...
for i in range(1, len(puzz) - 1):
	line = puzz[i]
	l = 0
	inlineNums = []
	while l < len(line):
		num = ""
		while l < len(line) and line[l] not in numbers: l += 1
		while l < len(line) and line[l] in numbers:
			num += line[l]
			l += 1
		if len(num): inlineNums.append((num, l - len(num), l - 1))
	for number, start, end in inlineNums:
		add = False
		if start == 0:
			if line[end + 1] in symbols: add = True
			for x in range(start, end + 2):
				if puzz[i - 1][x] in symbols or puzz[i + 1][x] in symbols:
					add = True
					break
		elif end == len(line) - 1:
			if line[start - 1] in symbols: add = True
			for x in range(start - 1, end + 1):
				if puzz[i - 1][x] in symbols or puzz[i + 1][x] in symbols:
					add = True
					break
		else:
			if line[start - 1] in symbols or line[end + 1] in symbols: add = True
			for x in range(start - 1, end + 2):
				if puzz[i - 1][x] in symbols or puzz[i + 1][x] in symbols:
					add = True
					break
		if add: total += int(number)
...
for i in range(len(puzz)):
	...
	for char in range(len(line)):
		if line[char] == "*": asterisks.append((char, i))
...
for x, y in asterisks:
	numSet = set()
	if y - 1 in gridMap:
		if x - 1 in gridMap[y - 1] and gridMap[y - 1][x - 1] not in numSet: numSet.add(gridMap[y - 1][x - 1])
		if x in gridMap[y - 1] and gridMap[y - 1][x] not in numSet: numSet.add(gridMap[y - 1][x])
		if x + 1 in gridMap[y - 1] and gridMap[y - 1][x + 1] not in numSet: numSet.add(gridMap[y - 1][x + 1])
	if y in gridMap:
		if x - 1 in gridMap[y] and gridMap[y][x - 1] not in numSet: numSet.add(gridMap[y][x - 1])
		if x + 1 in gridMap[y] and gridMap[y][x + 1] not in numSet: numSet.add(gridMap[y][x + 1])
	if y + 1 in gridMap:
		if x - 1 in gridMap[y + 1] and gridMap[y + 1][x - 1] not in numSet: numSet.add(gridMap[y + 1][x - 1])
		if x in gridMap[y + 1] and gridMap[y + 1][x] not in numSet: numSet.add(gridMap[y + 1][x])
		if x + 1 in gridMap[y + 1] and gridMap[y + 1][x + 1] not in numSet: numSet.add(gridMap[y + 1][x + 1])
	if len(list(numSet)) == 2:
		product = 1
		for n in numSet:
			product *= int(n)
		total += product
